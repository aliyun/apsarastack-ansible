...
def create_ess_group(module, ess_conn):
    params = {
        "ScalingGroupName": module.params['name'],
        "MaxSize": module.params['max_size'],
        "MinSize": module.params['min_size'],
        "DefaultCooldown": module.params['cooldown'],
        "RemovalPolicy.1": module.params['removal_policies'][0],
        "RemovalPolicy.2": module.params['removal_policies'][1],
        "VpcId": module.params['vpc_id']
    }
    index = 0
    for vswitch in module.params['vswitch_ids']:
        key = "VSwitchIds.%s" % (index + 1)
        params[key] = vswitch
    if module.params.get('db_instance_ids'):
        params['DBInstanceIds'] = module.params['db_instance_ids']
        
    try:
        response = do_common_request(
            ess_conn, "POST", "Ess", "2014-08-28", "CreateScalingGroup", body=params)
        if response["asapiSuccess"]:
            scalinggroup_id = response["ScalingGroupId"]
            return ess_conn.describe_groups(scaling_group_ids=[scalinggroup_id])[0]
...
def main():
    ...
    argument_spec.update(dict(
        ...
        vpc_id = dict(type=str, aliases=['vpc'])
    ))
    ...
    if state == 'present':
        if current is None:
            try:
                ...
                current = create_ess_group(module, ess)
