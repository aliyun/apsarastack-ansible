...
def do_get_images(module, image_id, wait, wait_timeout):
    if wait and isinstance(wait, int):
        time.sleep(wait)
    now = 0
    if not wait_timeout or not isinstance(wait_timeout, int):
        wait_timeout = 300
    while now < wait_timeout:
        images = describe_image(module, image_id)
        if images:
            return images
        else:
            time.sleep(10)
        ...
        module.fail_json(
            msg="Failed to get_image_detail timeout!  image_id: %s " % image_id)

def do_create_image(module, image: dict) -> dict:
    response = None
    try:
        response = do_common_request(
            ecs_connect(module), "POST", "Ecs", "2014-05-26", "CreateImage", body=image)
        return response
    except Exception as e:
        module.fail_json(
            msg="Failed to create_image: %s  params: %s  response:%s" % (e, image, response))
...
def create_image(module, ecs, snapshot_id, image_name, image_version, description, instance_id,
    ...
    image_params = {'ImageName': image_name, 'ImageVersion': image_version, 'Description': description,
                    'InstanceId': instance_id, 'DiskDeviceMapping': disk_mapping, 'ClientToken': client_token,
                    }
    response = do_create_image(module, image_params)
    image_id = None
    if response["asapiSuccess"]:
        changed = True
        image_id = response["ImageId"]
    return changed, image_id, describe_image(module, image_id, wait, wait_timeout)
