...
class Test(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self._slb_vsg_args["load_balancer_id"] = self._slb_args['id']
        self._slb_vsg_args["backend_servers"][0]["server_id"] = self._ecs_instance_args['id']
        result = run_module(slb_vsg_main, self._slb_vsg_args)
        self._slb_vsg_args["id"] = result["vserver_group"]["id"]
    ...
    def testSlbSvgInfo(self):
        svg_info_args = {
            "load_balancer_id": self._slb_args['id'],
            "vserver_group_ids": [self._slb_vsg_args['id'],],
            "name_prefix": "test"
        }
        result = run_module(ali_slb_vsg_info, svg_info_args)
        print(result)
        self.assertNotIn('failed', result)
        self.assertEqual(len(result['vserver_groups']), 1)
        self.assertEqual(result['vserver_groups'][0]['vserver_group_id'], self._slb_vsg_args['id'])
