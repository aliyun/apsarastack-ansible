...
class Test(unittest.TestCase):
    ...
    def __init__(self, methodName:str="runTest") -> None:
        ...
        self.name = "ansible_test_slb_server_%s" % uuid.uuid1()
        self._vpc_args = {
    ...
    def testSlbSvg(self):
        ...
        result = run_module(slb_vsg_main, svg_args)
        print(result)
        self.assertNotIn('failed', result)
        vserver_group = result['vserver_group']
        self.assertEqual(vserver_group['name'], svg_args['vserver_group_name'])
        self.assertEqual(len(vserver_group['backend_servers']), 1)
        self.assertEqual(vserver_group['backend_servers'][0]["server_id"], svg_args['backend_servers'][0]["server_id"])
