...
class Test(unittest.TestCase):
    ...
    def __init__(self, methodName:str="runTest") -> None:
        ...
        self.name = "ansible_test_rds_instance_%s" % uuid.uuid1()
        self._vpc_args = {
    ...
    def testCreateRdsInstance(self):
        rds_instance_args = {
        "engine": "MySQL",
        "engine_version": "5.7",
        ...
        result = run_module(rds_instance_main, rds_instance_args)
        self.assertEqual(result['changed'], True)
        instance_id = result['instances']['id']
        self.assertEqual(result['instances']["name"] , rds_instance_args["db_instance_name"])
        rds_instance_args = {
            "state": "restart",
