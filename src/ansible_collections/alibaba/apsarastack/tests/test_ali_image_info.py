...
from ansible_collections.alibaba.apsarastack.plugins.modules.ali_image import main as image_main
from ansible_collections.alibaba.apsarastack.plugins.modules.ali_image_info import main as image_info_main
...
class Test(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        print(self._ecs_args)
        result = run_module(instance_main, self._ecs_args)
        self._ecs_args["id"] = result['instances'][0]["id"]
        self._image_args["instance_id"] = self._ecs_args["id"]
        result = run_module(image_main, self._image_args)
        print(result)
        self._image_args["id"] = result["image"]["id"]
