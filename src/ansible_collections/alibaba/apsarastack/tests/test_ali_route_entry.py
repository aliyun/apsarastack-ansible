...
class Test(unittest.TestCase):
    ...
    def tearDown(self) -> None:
        ...
        try:
            ...
            pass
        try:
            run_module(security_group, security_group)
        except:
            pass
        try:
    ...
    def testDescribeRouteEntry(self):
        
        route_entry_args = {
            "route_table_id": self._vpc_args['route_table_id'],
            "destination_cidrblock": "172.16.20.0/24",
            "nexthop_id": self._ecs_args["id"],
            "nexthop_type": 'Instance',
            "name": "test_re"
        }
        result = run_module(route_entry_main, route_entry_args)
        self.assertNotIn('failed', result)
        self.assertFalse(result['changed'])
        self.assertNotEqual(result['route_entry'], {})
