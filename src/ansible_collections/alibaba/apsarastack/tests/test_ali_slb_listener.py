...
class Test(unittest.TestCase):
    ...
    def __init__(self, methodName:str="runTest") -> None:
        ...
        self.name = "ansible_test_slb_listener_%s" % uuid.uuid1()
        self._vpc_info = {
    ...
    def testCreateSlbListener(self):
        slb_listener_args = {
            "state": "present",
            "listener_port": 80,
        ...
        result = run_module(slb_listener_main, slb_listener_args)
        listener = result['listener']
        self.assertEqual(listener["listener_port"], slb_listener_args["listener_port"])
        self.assertEqual(listener["backend_server_port"], slb_listener_args["backend_server_port"])
        self.assertEqual(listener["sticky_session"], slb_listener_args["sticky_session"])
        slb_listener_args["state"] = "stopped"
        result = run_module(slb_listener_main, slb_listener_args)
        self.assertEqual(result['listener']["status"], slb_listener_args["state"])
        slb_listener_args["state"] = "absent"
        result = run_module(slb_listener_main, slb_listener_args)
