...
class Test(unittest.TestCase):
    ...
    def testCreateSlbServer(self):
        ...
        result = run_module(slb_server_main, slb_server_args)
        self.assertEqual(result['backend_servers'][0]["id"], slb_server_args["backend_servers"][0]["server_id"])
        self.assertEqual(result['load_balancer_id'], slb_server_args["load_balancer_id"])
        self.assertNotIn('failed', result)
        slb_server_args["state"] = "absent"
        result = run_module(slb_server_main, slb_server_args)
        self.assertNotIn('failed', result)
